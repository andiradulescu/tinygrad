...
def _prepare_jit_inputs(args, kwargs):
  ...
  # Realizing a contiguous view can leave an alias wrapper. Capture its storage, including any surrounding movement ops.
  for i, u in enumerate(input_uops):
    while (base:=u.base).op is Ops.CONTIGUOUS: u = u.substitute({base:base.src[0]})
    input_uops[i] = u
