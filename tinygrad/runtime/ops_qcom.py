...
class QCOMComputeQueue(HWQueue):
  ...
  def exec(self, prg:QCOMProgram, args_state:QCOMArgsState, global_size, local_size):
    ...
    if prg.NIR and prg.wgsz != 0xfc: args_state.buf.cpu_view().view(offset=prg.wgsz * 4, size=12)[:] = struct.pack("III", *local_size)
...
class QCOMArgsState(HCQArgsState):
  def __init__(self, buf:HCQBuffer, prg:QCOMProgram, bufs:tuple[HCQBuffer, ...], vals:tuple[int, ...]=()):
    ...
    ctypes.memset(self.buf.cpu_view().addr, 0, prg.kernargs_alloc_size)
    ...
    for cnst_val,cnst_off,cnst_sz in prg.consts_info:
      self.buf.cpu_view().view(offset=cnst_off, size=cnst_sz)[:] = cnst_val.to_bytes(cnst_sz, byteorder='little')

    if prg.samp_cnt > 0: self.buf.cpu_view().view(offset=prg.samp_off, size=len(prg.samplers) * 4, fmt='I')[:] = array.array('I', prg.samplers)
    if prg.NIR:
...
class QCOMProgram(HCQProgram['QCOMDevice']):
  def __init__(self, dev: QCOMDevice, obj: TinyELF):
    ...
    self.lib_gpu.cpu_view().view(size=self.image_size)[:] = self.image
...
class MSMIface:
  ...
  def prepare_submit(self, command:HCQBuffer, size:int, buffers:set[HCQBuffer]):
    ...
    submit_allocations = {command_allocation.handle:command_allocation}
    submit_allocations.update((allocation.handle, allocation) for allocation in map(self._resolve_allocation, buffers))
    allocations = list(submit_allocations.values())
    flags = msm_drm.MSM_SUBMIT_BO_READ | msm_drm.MSM_SUBMIT_BO_WRITE
    bos = (msm_drm.struct_drm_msm_gem_submit_bo * len(allocations))(*[
      msm_drm.struct_drm_msm_gem_submit_bo(flags=flags, handle=allocation.handle, presumed=allocation.iova) for allocation in allocations])
